...
(use-package org-latex-instant-preview
  ...
  :config
  (when IS-LINUX
    (setq org-latex-instant-preview-tex2svg-bin "/run/current-system/sw/bin/tex2svg")
    (setq org-latex-instant-preview-scale 10.0)
    )
  (when IS-MAC
    (setq org-latex-instant-preview-tex2svg-bin "~/.nix-profile/bin/tex2svg")
      )
  )
#+end_src
