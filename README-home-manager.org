...
* Overlay
** Python Overlay
#+begin_src nix :exports both :tangle  "~/.config/nixpkgs/home-manager/home-overlay/python-overlay.nix"
final: prev:
rec {
...
#+end_src
** sh
#+begin_src nix :async t :exports both :tangle "~/.config/nixpkgs/home-manager/programs/fish.nix"#{ config, pkgs, lib, ... }:
with lib;
{
  #fish
  config = with lib; mkMerge [
